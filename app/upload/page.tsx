    ...
    <main className="min-h-screen paper-bg flex flex-col">
      ...
      <header className="px-6 py-4 max-w-3xl mx-auto w-full flex items-center justify-between">
        ...
        <button
          ...
          className="font-serif text-xs text-[#8b8b8b] hover:text-[#4a5339] tracking-wider"
        >
          更换密钥
        </button>
      </header>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-6">
        <div className="max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="font-serif text-xs text-[#5a5a5a] tracking-wide">
          ...
          <div className="card-paper p-6 shadow-sm space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="王先生"
                    className="input-paper w-full px-3 py-2.5 rounded text-sm"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex gap-1 bg-[#faf7ef] border border-[#d4cdb8] rounded-full p-0.5">
                      <button
                        ...
                        className={`text-xs px-2 py-0.5 rounded-full transition-all ${calendarType === "solar" ? "bg-[#4a5339] text-[#f5f1e8]" : "text-[#8b8b8b]"}`}
                      >阳历</button>
                      <button
                        type="button"
                        onClick={() => setCalendarType("lunar")}
                        className={`text-xs px-2 py-0.5 rounded-full transition-all ${calendarType === "lunar" ? "bg-[#4a5339] text-[#f5f1e8]" : "text-[#8b8b8b]"}`}
                      >农历</button>
                    </div>
                  </div>
                  <input
                  ...
                  />
                  {calendarType === "lunar" && (
                    <p className="text-[10px] text-[#8b6f47] mt-1 font-serif tracking-wide">* 农历日期请换算后再填</p>
                  )}
                </div>
                <div>
              ...
              {preview ? (
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 bg-[#b85c3a] text-white rounded-full w-7 h-7 flex items-center justify-center text-xs hover:bg-[#a04a2a]"
                  >
                    ✕
                  </button>
                </div>
            ...
            {!canSubmit && !error && (
              <p className="font-serif text-xs text-center text-[#8b8b8b] tracking-wider">
                请补齐工位照片与出生日期时辰
              </p>
            )}
          </div>
